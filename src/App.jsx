...
function App() {
  const [screen, setScreen] = useState(() => {
    try {
      ...
      if (s) return JSON.parse(s).screen || "home";
    } catch {}
    return "home";
  });
  const [matchData, setMatchData] = useState(() => {
    try {
      const s = localStorage.getItem(APP_KEY);
      if (s) return JSON.parse(s).matchData || null;
    } catch {}
    return null;
  });
  const [lastMatchData, setLastMatchData] = useState(() => {
    try {
      const s = localStorage.getItem(APP_KEY);
      if (s) return JSON.parse(s).lastMatchData || null;
    } catch {}
    return null;
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        APP_KEY,
        JSON.stringify({ screen, matchData, lastMatchData }),
      );
    } catch {}
  ...
  const menuItems = [
    {
      icon: "🏏",
      label: "Gully Cricket",
      sub: "Start a new match",
      screen: "setup",
      color: "btn-icon-green",
      enabled: true,
    },
    {
      icon: "👤",
      label: "Players",
      sub: "Create & manage players",
      screen: "players",
      color: "btn-icon-blue",
      enabled: true,
    },
